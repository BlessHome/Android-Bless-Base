...
/**
 * <h3>一个简单易用的导航栏TitleBar</h3>
 * Created by aslai on 16-7-5.
 */
public class TitleBar extends ViewGroup implements View.OnClickListener {
    ...
    private void initView(Context context) {
        mNavigation = new TextView(context);
        ...
        mNavigation.setTextSize(DEFAULT_ACTION_TEXT_SIZE);
        mNavigation.setSingleLine();
        mNavigation.setGravity(Gravity.CENTER_VERTICAL);
        mNavigation.setPadding(mOutPadding, 0, mOutPadding, 0);
        ...
        mActionLayout.setPadding(mOutPadding, 0, mOutPadding, 0);

        addView(mNavigation, layoutParams);
        addView(mCenterLayout);
        addView(mActionLayout, layoutParams);
    ...
    public void setImmersive(boolean immersive) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT)
        {
            ...
            Log.d(TitleBar.class.getSimpleName(), "SDK VERSION is need KITKAT");
        }
    }
    ...
    public void setNavigationIcon(int iconId) {
        mNavigation.setCompoundDrawablesWithIntrinsicBounds(iconId, 0, 0, 0);
    }

    public void setNavigationIcon(@Nullable Drawable icon) {
        mNavigation.setCompoundDrawablesWithIntrinsicBounds(icon, null, null, null);
    }

    public void setNavigationClickListener(OnClickListener l) {
        mNavigation.setOnClickListener(l);
    }

    public void setNavigationText(CharSequence title) {
        mNavigation.setText(title);
    }

    public void setNavigationText(int resid) {
        mNavigation.setText(resid);
    }

    public void setNavigationTextSize(float size) {
        mNavigation.setTextSize(size);
    }

    public void setNavigationTextColor(int color) {
        mNavigation.setTextColor(color);
    }

    public void setNavigationVisible(boolean visible) {
        mNavigation.setVisibility(visible ? View.VISIBLE : View.GONE);
    }
    ...
    public View addAction(Action action) {
        final int index = mActionLayout.getChildCount();
    ...
    public View addAction(Action action, int index) {
        ...
        mActionLayout.addView(view, index, params);
        return view;
    }
    ...
    public void removeActionAt(int index) {
        mActionLayout.removeViewAt(index);
    }
    ...
    public void removeAction(Action action) {
        int childCount = mActionLayout.getChildCount();
        for (int i = 0; i < childCount; i++) {
            View view = mActionLayout.getChildAt(i);
            if (view != null) {
                final Object tag = view.getTag();
    ...
    public int getActionCount() {
        return mActionLayout.getChildCount();
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        measureChild(mNavigation, widthMeasureSpec, heightMeasureSpec);
        measureChild(mActionLayout, widthMeasureSpec, heightMeasureSpec);
        if (mNavigation.getMeasuredWidth() > mActionLayout.getMeasuredWidth()) {
            mCenterLayout.measure(
                    MeasureSpec.makeMeasureSpec(mScreenWidth - 2 * mNavigation.getMeasuredWidth(), MeasureSpec.EXACTLY)
                    , heightMeasureSpec);
        } else {
            mCenterLayout.measure(
                    MeasureSpec.makeMeasureSpec(mScreenWidth - 2 * mActionLayout.getMeasuredWidth(), MeasureSpec.EXACTLY)
                    , heightMeasureSpec);
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        mNavigation.layout(0, mStatusBarHeight, mNavigation.getMeasuredWidth(), mNavigation.getMeasuredHeight() + mStatusBarHeight);
        mActionLayout.layout(mScreenWidth - mActionLayout.getMeasuredWidth(), mStatusBarHeight,
                mScreenWidth, mActionLayout.getMeasuredHeight() + mStatusBarHeight);
        if (mNavigation.getMeasuredWidth() > mActionLayout.getMeasuredWidth()) {
            mCenterLayout.layout(mNavigation.getMeasuredWidth(), mStatusBarHeight,
                    mScreenWidth - mNavigation.getMeasuredWidth(), getMeasuredHeight());
        } else {
            mCenterLayout.layout(mActionLayout.getMeasuredWidth(), mStatusBarHeight,
                    mScreenWidth - mActionLayout.getMeasuredWidth(), getMeasuredHeight());
        }
